...
import type { Principal } from '@passlock/shared/src/schema'
...
import { type GetData, noopLogger, type PostData } from '../testUtils'
...
const buildMocks = (registered: boolean) => {
  const getData: GetData = vi
    .fn()
    .mockImplementationOnce(() => E.succeed({ registered }))
...
export async function runEffect<O>(effect: In<O>, registered: boolean): Out<O> {
  ...
  const tenancyTest = Layer.succeed(
    Tenancy,
    Tenancy.of({ tenancyId, clientId }),
  )
