...
import { Context, Effect as E, Layer, flow, Match } from "effect"
...
export const preConnect = (request: PreConnectRequest) => E.gen(function* (_) {
  const logger = yield* _(PasslockLogger)
  ...
  const matcher = Match.type<PreConnectRequest>().pipe(
    Match.when({ operation: 'registration' }, () => [registrationService.preConnect, userService.preConnect, pingTokenEndpoint]),
    Match.when({ operation: 'authentication' }, () => [authenticationService.preConnect, pingTokenEndpoint]),
    Match.when({ operation: 'all' }, () => [registrationService.preConnect, authenticationService.preConnect, pingTokenEndpoint]),
    Match.exhaustive
  )
...
export const WarmerServiceLive = Layer.effect(
  ...
  E.gen(function* (_) {
    const context = yield* _(
      E.context<Get | NetworkService | AuthenticationService | UserService | PasslockLogger | RegistrationService>(),
    )
